...
class ChefController extends Controller
{
    public function index(Request $request)
    {
       // $users = User::query();
        ...
        if($request->ajax())
        {
            return response()->json( view('admin.wholeseller.user_data', compact('users'))->render());
        }
        return view('admin.chef.list' , compact(['users']));
    }
    ...
    public function chefs(Request $request)
    {
        ...
        if(!empty($users))
        {
            ...
            {
                $url = route('admin.chef.get', ['user_id' => $user->id]);
                $statusUrl = route('admin.chef.status.change', ['user_id' => $user->id]);
                $checked = $user->status == 1 ? 'checked' : '';
                $user_image =  $user->photo== null ? url('/admin_assets/img/logo.png') : $user->photo;
                    
                $nestedData['id'] = $user->id;
                $nestedData['photo'] = "<img width='50' height='50' src=' $user_image' class='rounded-circle shadow-2' alt='Image'>";
                $nestedData['name'] = $user->name;
                $nestedData['email'] = $user->email;
                $nestedData['total_subscribers'] = $user->total_subscribers;
